        ...
        <form class="form-width" method="POST" action="{{route('products.update', $product->id)}}">
            ...
            <div class="form-row">
              ...
              <div class="form-group col-md-6">
                ...
                <input type="Product Value" class="form-control" id="inputProductValue4" placeholder="Product Value" name="value" value="{{$product->value}}">
              </div>
            </div>
            <div class="form-group">
              <label for="example-date-input" class="col-7 col-form-label">Date of Purchase</label>
              <input class="form-control" type="date" name="date_of_purchase" value="today_parsed" id="example-date-input" value="{{$product->date_of_purchase}}">
            </div>
            <div class="form-group">
              <label for="inputComment">Comment</label>
              <input type="text" class="form-control" id="inputComment" placeholder="Add Product Comment" name="comment" value="{{$product->comment}}">
            </div>
            <div class="form-row">
              <div class="form-group col-md-3">
                <label for="inputSerialKey">Serial Key</label>
                <input type="text" class="form-control" id="inputSerialKey" name="serial_key" value="{{$product->serial_key}}">
              </div>
              <div class="form-group col-md-3">
                  <label class="mr-sm-2" for="inlineFormCustomSelect">Category</label>
                  <select class="custom-select mr-sm-2" id="inlineFormCustomSelect"  name='category_id'>
                    <option selected>{{$product->categories['name']}}</option>
